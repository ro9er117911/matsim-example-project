"""
Generate test population for MATSim with 500 agents:
- 250 PT agents (100 single-line + 150 transfer) - PT-ONLY (no car availability)
- 200 car agents
- 50 walk agents

Key Features:
- PT agents are PT-ONLY (no car in vehicles attribute)
- 150 transfer agents to demonstrate SwissRailRaptor transfer capability
...
# ========== PT TRANSFER ROUTES ==========
# Expanded list for 150 transfer agents
PT_TRANSFER_ROUTES = [
...
# Repeat routes to reach 150 total
while len(PT_TRANSFER_ROUTES) < 150:
    PT_TRANSFER_ROUTES.extend(PT_TRANSFER_ROUTES[:min(150 - len(PT_TRANSFER_ROUTES), len(PT_TRANSFER_ROUTES))])

# Single-line PT routes (expand to 100)
...
# Repeat to reach 100
while len(PT_SINGLE_LINE_ROUTES) < 100:
    PT_SINGLE_LINE_ROUTES.extend(PT_SINGLE_LINE_ROUTES[:min(100 - len(PT_SINGLE_LINE_ROUTES), len(PT_SINGLE_LINE_ROUTES))])
...
def generate_transfer_pt_agent(agent_id, home_station, transfer_station_1, transfer_station_2, work_station, departure_hour, departure_min):
	...
	<person id="pt_transfer_agent_{agent_id:03d}">
		<plan selected="yes">
			<activity type="home" x="{home_x:.2f}" y="{home_y:.2f}" end_time="{morning_depart}" />
			<leg mode="pt" />
			<activity type="work" x="{work_x:.2f}" y="{work_y:.2f}" end_time="{evening_depart}" />
...
def generate_car_agent(agent_id, home_station, work_station, departure_hour, departure_min):
	...
	<person id="car_agent_{agent_id:03d}">
		<plan selected="yes">
			<activity type="home" x="{home_x:.2f}" y="{home_y:.2f}" end_time="{morning_depart}" />
			<leg mode="car" />
			<activity type="work" x="{work_x:.2f}" y="{work_y:.2f}" end_time="{evening_depart}" />
...
def generate_walk_agent(agent_id, home_station, work_station, departure_hour, departure_min):
	...
	<person id="walk_agent_{agent_id:03d}">
		<plan selected="yes">
			<activity type="home" x="{home_x:.2f}" y="{home_y:.2f}" end_time="{morning_depart}" />
			<leg mode="walk" />
			<activity type="work" x="{work_x:.2f}" y="{work_y:.2f}" end_time="{evening_depart}" />
...
# Generate 100 single-line PT agents (PT-ONLY)
print("=" * 70)
print("GENERATING 500 AGENTS POPULATION")
print("=" * 70)
print("\n[1/4] Generating 100 single-line PT agents (PT-ONLY)...")
pt_single_created = 0
pt_single_skipped = 0
...
for i in range(100):
    ...
    if is_valid_trip(home_station, work_station, 'pt'):
        hour, minute = random.choice(PEAK_MORNING if i < 70 else OFF_PEAK_MORNING)
...
print(f"  ✓ Created: {pt_single_created}/100")
if pt_single_skipped > 0:
    print(f"  ⚠ Skipped: {pt_single_skipped}")

# Generate 150 PT transfer agents (PT-ONLY)
print("\n[2/4] Generating 150 PT transfer agents (PT-ONLY)...")
...
for i in range(150):
    if i < len(PT_TRANSFER_ROUTES):
        ...
        pt_transfer_skipped_list.append(f"pt_transfer_agent_{i+101:03d}")
        continue
    ...
    if total_time <= MAX_TRIP_TIME_MINUTES:
        hour, minute = random.choice(PEAK_MORNING if i < 105 else OFF_PEAK_MORNING)
        xml_content += generate_transfer_pt_agent(i + 101, home_station, transfer_station_1, transfer_station_2, work_station, hour, minute)
        xml_content += '\n'
        pt_transfer_created += 1
    else:
        pt_transfer_skipped += 1
        pt_transfer_skipped_list.append(f"pt_transfer_agent_{i+101:03d} ({total_time:.1f}min)")

print(f"  ✓ Created: {pt_transfer_created}/150")
...
# Generate 200 car agents
print("\n[3/4] Generating 200 car agents...")
...
for i in range(200):
    ...
    if attempts < max_attempts:
        hour, minute = random.choice(PEAK_MORNING if i < 140 else OFF_PEAK_MORNING)
        xml_content += generate_car_agent(i + 251, home_station, work_station, hour, minute)
        xml_content += '\n'
        car_created += 1
    else:
        car_skipped += 1

print(f"  ✓ Created: {car_created}/200")
if car_skipped > 0:
    print(f"  ⚠ Skipped: {car_skipped}")

# Generate 50 walk agents
print("\n[4/4] Generating 50 walk agents...")
walk_routes = [
...
# Repeat to get 50
while len(walk_routes) < 50:
    walk_routes.extend(walk_routes[:min(50 - len(walk_routes), len(walk_routes))])
...
for i in range(50):
    ...
    if is_valid_trip(home_station, work_station, 'walk'):
        ...
        xml_content += generate_walk_agent(i + 451, home_station, work_station, hour, minute)
        xml_content += '\n'
        walk_created += 1
    else:
        walk_skipped += 1

print(f"  ✓ Created: {walk_created}/50")
if walk_skipped > 0:
...
import os
output_file_base = os.getenv('POPULATION_OUTPUT_PATH', '5000_disatar/working_temp/gtfs_0300_0618/population_subway_500agents.xml')
output_file_gz = output_file_base + '.gz'

os.makedirs(os.path.dirname(output_file_base), exist_ok=True)

# Write plain XML first
with open(output_file_base, 'w', encoding='utf-8') as f:
    f.write(xml_content)

# Compress to .gz
with open(output_file_base, 'rb') as f_in:
    with gzip.open(output_file_gz, 'wb') as f_out:
        f_out.writelines(f_in)

# Remove plain XML (keep only .gz)
os.remove(output_file_base)
...
print("=" * 70)
print(f"\n✓ Output: {output_file_gz}")
print(f"\nAgent Summary:")
print(f"  PT single-line (PT-ONLY):  {pt_single_created}/100")
print(f"  PT transfer (PT-ONLY):     {pt_transfer_created}/150")
print(f"  Car agents:                 {car_created}/200")
print(f"  Walk agents:                {walk_created}/50")
print(f"  ────────────────────────────────")
print(f"  TOTAL:                      {total_created}/500")
...
print(f"  ✓ Output compressed as .xml.gz")
print(f"\nNext Steps:")
