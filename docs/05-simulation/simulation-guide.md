...
## Disaster Evacuation Scenario (5000 agents)

This repo includes a tsunami/flood evacuation scenario under `5000_disatar/`.
It uses **time-variant road closures** (`changeEvents.xml`) and a **staggered departure window**
to simulate evacuation dynamics (see `5000_disatar/05_combined_evac/WORKFLOW.md`).

### Recommended Runs

```bash
# Recommended baseline with SimWrapper post-processing
scripts/run_simulation_with_via_export.sh \
  5000_disatar/05_combined_evac/config_optimized_iter10.xml

# Staggered departure pipeline (produces dashboards + filtered GeoJSON)
./5000_disatar/05_combined_evac/run_staggered_iter10_pipeline.sh
```

### Key Inputs (defined in config)

- **Network**: `inputNetworkFile` (e.g., `scenarios/corridor/500_300-618/network-with-pt-metro-v7-carscc.xml.gz`)
- **Population**: `inputPlansFile` (e.g., `5000_disatar/05_combined_evac/input/population_5000_staggered.xml`)
- **PT Schedule/Vehicles**: `transitScheduleFile`, `vehiclesFile`
- **Time-variant closures**: `inputChangeEventsFile` (e.g., `input/tsunami_changeEvents_staggered.xml`)

### Output & Analysis

- Output folder is set by `controller.outputDirectory` (e.g., `output_staggered_iter10`).
- Run dashboards:
  ```bash
  tools/run_dashboard_pipeline.sh output_staggered_iter10
  ```

### Common Disaster Settings

- `network.timeVariantNetwork = true` (required for closures)
- `transit.usingTransitInMobsim = false` (teleport PT if vehicles get stuck)
- Use 8–16 GB JVM heap for 5k agents (see `scripts/run_simulation_with_via_export.sh`)
...
- [Agent Generation Guide](../04-population/agent-generation.md) - How to create custom populations
- [Via Export Guide](via-export.md) - Detailed Via platform export instructions
- [Output Analysis Guide](../07-analysis/output-analysis.md) - Analyzing simulation results
- [Configuration Reference](../08-configuration/configuration-reference.md) - MATSim configuration options
- [Troubleshooting Guide](../10-troubleshooting/troubleshooting.md) - Common issues and solutions
