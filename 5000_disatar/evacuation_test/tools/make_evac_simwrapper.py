...
def main():
    ...
    with gzip.open(args.events_gz, "rb") as f:
        for ev, el in ET.iterparse(f, events=("end",)):
            ...
            if typ == "actend" and el.attrib.get("actType") == "pre-evac":
                pre[pid] = (float(el.attrib["x"]), float(el.attrib["y"]), float(el.attrib["time"]))
            elif typ == "actstart" and el.attrib.get("actType") == "post-evac":
    ...
    print(f"Found {len(pre)} pre-evac, {len(post)} post-evac, {len(arrival)} arrivals.")
    ...
    if df.empty:
        print("No matched pre-evac/post-evac pairs found in events")
        return
    ...
    # 3) flow per minute (evacuation speed proxy)
    ...
    # 4) grid mean evacuation time -> .xyt.csv
    N = args.cells
    ...
    for v in map(float, qs):
        if v > last + 1e-9:
            bps.append(v); last = v
    # Ensure we have something in bps even if empty
    ...
    dash = {
        "header": {
            "tab": "Evacuation",
            "title": "Evacuation summary (from MATSim events)",
