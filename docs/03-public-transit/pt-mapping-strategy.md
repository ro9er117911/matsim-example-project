...
**关键要求**：必须完成 GTFS 验证前的所有检查清单（见 `gtfs-mapping-guide.md`）
...
### GTFS 数据检查
```bash
# 运行 gtfs-mapping-guide.md 中的验证脚本
python3 << 'EOF'
import pandas as pd
...
映射前**务必**把路网清理到单一连通分量，避免之后出现 “No route found”：

```bash
# 使用项目内置工具：添加 pt 模式并只保留最大 car 分量
./mvnw exec:java \
...
gunzip -c <output_network_clean.xml.gz> | grep -c '<node '
```
...
详见: `../05-simulation/early-stop-strategy.md` 中的资源监控和超时管理
...
# 摘要可疑项（超长接驳、断链、人工链接过多）
rg "WARN|ERROR|suspicious|far away|artificial" pt2matsim/output_v1/check_plausibility.log | head -40

# 将可疑停靠点导出清单便于修复
python3 - <<'PY'
from pathlib import Path
log = Path("pt2matsim/output_v1/check_plausibility.log")
issues = []
for line in log.read_text().splitlines():
    lower = line.lower()
    if any(key in lower for key in ["far away", "artificial", "no link", "cannot map", "suspicious"]):
        issues.append(line)
...
### 4.4 映射后清理与修复（对应官方 “Check-Mapped-Schedule-Plausibility”）

- **距离过远 / 人工链接过多**：若日志出现 “far away from link” 或人工链接占比高，调高映射参数后重跑：`maxLinkCandidateDistance` 400-500、`nLinkThreshold` 12-15、`maxTravelCostFactor` 15-20、`candidateDistanceMultiplier` 3.0-4.0。
- **断线 / 无法路由**：先重新用 `PrepareNetworkForPTMapping` 清理路网，确保 car+pt 连通，再重跑映射；必要时移除极小分量。
- **异常线路几何**：把 `check_plausibility.log` 中的可疑路线 ID 导出，在 QGIS 叠加 OSM 与河流边界检查是否穿河、掉头；可将问题路线/停靠点列入过滤清单后重跑映射。
- **迭代修复顺序**：清理路网 → 调参 → 重跑映射 → 再跑 plausibility 检查，直到 `check_plausibility.log` 只剩零星警告。
- **进入仿真前**：在 `config.xml` 把 network/transitSchedule/vehicles 指向清理后的文件，避免出现 “No route found” 或 PT 断线。
...
详细的资源管理策略见: `../05-simulation/early-stop-strategy.md`
...
- **GTFS 数据验证**: `gtfs-mapping-guide.md`
- **资源管理和超时**: `../05-simulation/early-stop-strategy.md`
- **整体项目架构**: `CLAUDE.md`
- **下一个 agent 任务**: [pt/next-agent-instructions.md](pt/next-agent-instructions.md)
